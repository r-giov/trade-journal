...
export default function Nav({ onSignOut }) {
  ...
  return (
    <nav style={{ display:'flex', alignItems:'center', padding:'0 24px', height:54, borderBottom:'1px solid #e2e8f0', background:'#fff', position:'sticky', top:0, zIndex:100, boxShadow:'0 1px 3px rgba(0,0,0,0.06)' }}>
      ...
      <button onClick={onSignOut} style={{ background:'none', border:'1px solid #e2e8f0', borderRadius:6, color:'#94a3b8', fontSize:11, padding:'5px 12px', cursor:'pointer', fontFamily:'var(--font-mono)' }}>
        sign out
      </button>
    </nav>
  )
